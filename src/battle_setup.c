...
const struct RematchTrainer gRematchTable[REMATCH_TABLE_ENTRIES] =
{
    ...
    [REMATCH_JACKI] = REMATCH(TRAINER_LANCE_1, TRAINER_CHAMPIONBLUE_2, TRAINER_RIVAL_CHIKORITA_1, TRAINER_RIVAL_CHIKORITA_2, TRAINER_RIVAL_CHIKORITA_3, ROUTE123),
...
static u8 GetSumOfEnemyPartyLevel(u16 opponentId, u8 numMons)
{
    ...
    u8 sum;
    u32 count = numMons;
    const struct Trainer *trainer = &gTrainers[opponentId];

    

    // Choose party and partySize based on difficulty flag
    union TrainerMonPtr partyPtr;
    u8 partySize;
    if (gSaveBlock1Ptr->tx_Challenges_TrainerDifficulty)
    {
        partyPtr = trainer->partyHard;
    ...
    case 0:
        {
            ...
            party = partyPtr.NoItemDefaultMoves;
            for (i = 0; i < count; i++)
    ...
    case F_TRAINER_PARTY_CUSTOM_MOVESET:
        {
            ...
            party = partyPtr.NoItemCustomMoves;
            for (i = 0; i < count; i++)
                sum += GetScaledLevel(party[i].lvl);
        }
        break;
    case F_TRAINER_PARTY_HELD_ITEM:
        {
            ...
            party = partyPtr.ItemDefaultMoves;
            for (i = 0; i < count; i++)
                sum += GetScaledLevel(party[i].lvl);
        }
        break;
    case F_TRAINER_PARTY_CUSTOM_MOVESET | F_TRAINER_PARTY_HELD_ITEM:
        {
            ...
            party = partyPtr.ItemCustomMoves;
            for (i = 0; i < count; i++)
                sum += GetScaledLevel(party[i].lvl);
        }
        break;
...
static u8 GetTrainerBattleTransition(void)
{
    ...
    if (gTrainers[gTrainerBattleOpponent_A].trainerClass == TRAINER_CLASS_ELITE_FOUR)
    {
        ...
        if (gTrainerBattleOpponent_A == TRAINER_KAREN_2)
            return B_TRANSITION_DRAKE;
        if (gTrainerBattleOpponent_A == TRAINER_WILL_ROUTE39)
            return B_TRANSITION_SIDNEY;
        if (gTrainerBattleOpponent_A == TRAINER_WILL_ROUTE39_STRONG)
            return B_TRANSITION_SIDNEY;
        return B_TRANSITION_CHAMPION;
    }
    ...
    {
        if (gTrainerBattleOpponent_A == TRAINER_CHAMPIONBLUE_2)
            return B_TRANSITION_BLUE;
        return B_TRANSITION_CHAMPION;
    }
